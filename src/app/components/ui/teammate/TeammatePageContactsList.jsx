	...
	return !isLoading && isSuccess ? (
		<article className={contactsListClass}>
			<div className='flex items-center justify-around'>
				<span className='block text-3xl font-bold'>Социальные Сети</span>

				<UnorderedList listClass='flex justify-center'>
					{Object.keys(contacts).map((contactName) => (
						<a key={contacts[contactName]._id} href={contacts[contactName].urlPath}>
							<svg
							...
							>
								<path
									d={
										contactsData.find(
											(contactsItem) => contactsItem._id === contacts[contactName]._id,
										).svgPath
									}
								/>
							</svg>
